...
public:
	...
	/// </summary>
	Clustering_Algorithm(std::vector<Point> _points) : points(_points){};
...
public:
	...
	KMEANS(std::vector<Point> _points) : Clustering_Algorithm(_points) { init_values(); }
...
public:
	...
	GREEDY_KMEANS(std::vector<Point> _points, int _z = -1) : KMEANS(_points), z(_z) {}
...
public:
	...
	FLSPP(std::vector<Point> _points, int _z = -1) : GREEDY_KMEANS(_points, _z) {}
